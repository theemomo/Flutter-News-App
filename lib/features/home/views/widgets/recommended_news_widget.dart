  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final parsedDate = DateTime.parse(articles[index].publishedAt ?? DateTime.now().toString());
        final formattedDate = DateFormat.yMMMd().format(parsedDate);
        return InkWell(
          onTap: () {
            Navigator.of(context).pushNamed(AppRoutes.articleRoute, arguments: articles[index]);
          },
          child: Padding(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        articles[index].source!.name ?? "no-source",
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: Theme.of(
                          context,
                        ).textTheme.bodySmall!.copyWith(color: AppColors.gry),
                      ),
                      ...
                      const SizedBox(height: 10),
                      Text(
                        "${articles[index].author ?? "no-author"} • $formattedDate",
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: Theme.of(
                          context,
